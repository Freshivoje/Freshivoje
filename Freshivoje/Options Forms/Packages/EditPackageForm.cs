...
namespace Freshivoje.Options_Forms
{
    public partial class EditPackageForm : Form
    {
        ...
        public EditPackageForm(Package package)
        {
            ...
            packageCategoryTxtBox.Text = package._packageCategory.ToString();
            packageWeightTxtBox.Text = package._weight.ToString();
            packageProducerTxtBox.Text = package._producer.ToString();
            packageStatusCmbBox.Text = package._status.ToString();
        ...
        private void editBtn_Click_1(object sender, EventArgs e)
        {
            ...
            {
                CommandText = "UPDATE `packaging` SET `capacity` = @packageCapacity, `price` = @packagePrice,`category` = @packageCategory, `quantity` = @packageQuantity, `weight` = @packageWeight, `producer` = @packageProducer, `status` = @packageStatus WHERE `id_packaging` = @packagingId"
